...
- [ ] **Phase 76: Connect Stabilization + FLAC24 Integration** — Die Bugs und fehlenden Integrationen, die ein nutzbares v4.0 blockieren. (1) **Connect-Mode-Bugs:** #159 Player hängt in BUFFERING nach Connect-Deselect im Pause-Zustand, #158 Playback-Crash auf Gruppen-Playern (pause→skip→play), #150 Audio-Key-Timeout nach AP-Reconnect killt Playback, #149 Health-Check restartet Daemon mitten im Playback (Idle-Guard fehlt), #131 Sync-Group-Stutter bei gemischten Player-Typen, #128 Progress-Bar-Lag beim Connect-Handoff, #151 Player-Power-State nicht restauriert nach Connect-Session-Ende, Window 5 ~8s Reconnect-Lücke beim Track-Wechsel (Ring nicht geleert nach flush-disconnect), Auto-Play im Connect-Mode nach LMS-Restart ohne User-Aktion. (2) **Phase-73-Live-Verifikation:** Windows 1-4 (pause/skip/seek/volume, DaemonManager, SoloistWS, Browse Sequential Playback) sind alle "unrun-verify" — nie live getestet. (3) **FLAC24/Audio-Pipeline-Integration:** spoton-helper `patch` (Phase 74) tatsächlich verdrahten — Lifetime-Timestamp + FLAC24-Enum-Patching in den Daemon-Start integrieren, gepatchtes Binary nutzen; Direct-FLAC an FLAC-fähige Player (z.B. squeezelite) statt S32LE→FLAC-Transcoding in custom-convert.conf. (4) **Browse/Playback-Control:** #161 Play All/Queue auf flachen Tracklisten in Material Skin, #94 Browse-Kontextmenü-Parität mit TrackInfo, #135 Connect-Queue ("Up Next") in LMS, Window 6 search() Offset-Guard-Scope (Phase-75-Rest).
  **Depends on:** Phase 74 (spoton-helper `patch`), Phase 75 (SpClient)
  **Plans:** 8 plans (Wave 1: 76-01 ∥ 76-02 ∥ 76-03 · Wave 2: 76-04 ∥ 76-05 ∥ 76-06 · Wave 3: 76-07 ∥ 76-08)
