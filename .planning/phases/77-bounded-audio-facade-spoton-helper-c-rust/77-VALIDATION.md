...
(final per revised plans: **15** — ladder 9 → 10 → 12 → 13 → 15).
...
- **After every plan wave:** `prove -l t/*.t` — this phase's edits span the C ring/flush logic and 4+ Perl modules, so cross-module regressions (notably the `soloistBrowseActive` gate) are the main risk targeted runs miss. Wave 2 note (77-REVIEWS.md LOW): run the full suite at the wave MERGE, after both 77-04 and 77-05 complete — 77-04 edits t/29, and a full-suite run concurrent with those edits can race a half-edited file; 77-05's own verification deliberately omits the full suite
...
| 77-01/T2 | 77-01 | 1 | D-02/D-03 + D-11 | T-77-02/T-77-03 | Armed flush keeps client attached; no pre-seek bytes cross the socket (byte-content memcmp); POST /boundary resets a leaked arm so the drain gate cannot wedge and boundary EOF still fires (77-REVIEWS.md HIGH) | unit (C, tracer) | `make -C Plugins/SpotOn/Bin/fake-libpulse test 2>&1 \| grep -c '^ok:' \| grep -qx 12` | ❌ 2 new cases (`ok: seek-armed flush keeps client attached and serves only post-flush bytes`, `ok: leaked arm cleared by boundary and client closes at watermark`) | ⬜ pending |
...
| 77-04/T1 | 77-04 | 2 | D-02/D-05/D-10 | T-77-08..T-77-10 | Arm-then-command dispatch (callback-ordered) in both getNextTrack branches; idle daemon AND paused/stopped session un-armed (stale-lastTrackId gate); seek keys on defined-0 and appends a play resume (77-REVIEWS.md MEDIUMs) | unit (Perl) | `prove -l t/29_soloist_browse.t t/05_perl_syntax.t 2>&1 \| tail -1 \| grep -q 'Result: PASS'` | ❌ extend t/29 (+8 rows incl. SimpleAsyncHTTP stub + FakeSoloistWs sessionPaused accessor) | ⬜ pending |
...
| 77-05/T1 | 77-05 | 2 | D-04 | T-77-12 | Rapid-skip: last `POST /boundary` watermark wins (byte-count assertion) | unit (C) | `make -C Plugins/SpotOn/Bin/fake-libpulse test 2>&1 \| grep -c '^ok:' \| grep -qx 13` | ❌ new case (`ok: rapid-skip last boundary wins`) | ⬜ pending |
| 77-05/T2 | 77-05 | 2 | D-05 + OQ1 | T-77-11 | Takeover mid-armed-window serves zero pre-flush bytes; double-arm/double-flush survives; third un-armed flush disconnects (D-12 restored). Test A written against actual takeover behavior (dead `was_supersede` flag — 77-REVIEWS.md LOW) | unit (C) | `make -C Plugins/SpotOn/Bin/fake-libpulse test 2>&1 \| grep -c '^ok:' \| grep -qx 15` | ❌ 2 new cases | ⬜ pending |
...
| 77-06/T1 | 77-06 | 3 | D-07 + D-08 | — | Live UAT: 78-UAT test 10 pass, repeated seek, rapid-skip, daemon-crash clean EOF, session-end → new-album start (stale-arm probe, 77-REVIEWS.md MEDIUM) | manual (checkpoint:human-verify) | manual — see 77-06 Task 1 items 1-6 | ❌ manual-only by design | ⬜ pending |
...
- [x] Update the `ok:` count assertion from the stale `6` to the current `9` — RESOLVED: all plan verify commands use the 9 baseline + added cases (10 → 12 → 13 → 15; ladder revised after 77-REVIEWS.md — 77-01/T2 now lands TWO cases, the tracer plus the leaked-arm boundary-reset test); Open Question 1 (double-flush) resolved by design as a saturating arm COUNTER with a boundary-reset lifecycle (77-01/T2), characterized by test in 77-05/T2
...
| Session end → new album starts within ~2s | D-05 + A2 (77-REVIEWS.md MEDIUM) | Whether Soloist flushes on play-from-stopped is unverifiable against the closed-source binary; stale lastTrackId makes this the mainstream arm-leak trigger | Play to natural session end (daemon `stopped`), then start a different album from Browse; confirm audio starts within ~2s with no silent stall (77-06 UAT item 6) |
