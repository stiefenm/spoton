...
### Added
- **24-bit FLAC playback for the Soloist backend** — the audio pipeline now preserves Soloist's full output precision end to end (S32LE instead of a 16-bit down-conversion), and FLAC-capable players (e.g. squeezelite) automatically receive 24-bit FLAC through a new `soc flc` transcoding rule. The per-player Stream Format setting resolves capability-based under Soloist: Auto picks FLAC when the player supports it, PCM otherwise. The rule is shared with the librespot backend (16-bit there), so explicitly selecting FLAC now produces real FLAC instead of silently falling back to PCM.
- **Explicit MP3 stream format now actually forces MP3** — a dedicated `smp` rule (via the system `lame` binary) serves both backends; if `lame` is missing, LMS shows a clear stream error instead of silently playing a different format.
- **"Up Next" Connect queue in the SpotOn menu** — shows the currently playing track and the live Spotify Connect queue on demand (exactly one API request per menu open, no polling), for both backends. ([#135](https://github.com/stiefenm/spoton/issues/135))
- **Play All / Add to Queue on flat track lists** — Recently Played, Top Tracks, and Liked Songs now expose Material Skin's hover play actions. ([#161](https://github.com/stiefenm/spoton/issues/161))
- **Browse "More" menu matches the Now Playing track menu** — track and episode context menus in Browse now come from the same Track Info framework as Now Playing, adding Add to Playlist and the standard LMS items that were previously missing. ([#94](https://github.com/stiefenm/spoton/issues/94))
- **Backend-aware Stream Format dropdown** — OGG passthrough (librespot-only) is hidden from the per-player Stream Format setting when the Soloist backend is active; a stored OGG choice is preserved and restored when switching back to librespot.

- **Browse/Search/Library unified onto spclient.spotify.com via login5** — stored-credential accounts (ZeroConf/Keymaster/Soloist pairing) now serve Browse, Search, and Library reads through Spotify's internal spclient API instead of the public Web API, with automatic fallback to the Web API for PKCE-only accounts and any spclient-side failure. Search now returns up to 20 track results (double the Web API's Dev-Mode cap of 10), previously-removed `popularity`/`label` metadata fields are restored, and Liked Songs load without Web-API paging. Library writes (save/remove tracks/shows, add-to-playlist) and player control continue to use the Web API unchanged. A standalone `tools/spclient-smoke.pl` script is available for manual verification of the login5 + spclient chain without a running LMS instance.
- **`spoton-helper` binary for Soloist** — a new bundled Rust helper (`Bin/<arch>/spoton-helper`, x86_64/arm64/arm32, CI cross-built alongside the Soloist backend) with two subcommands: `patch` applies the Lifetime-timestamp and FLAC24-quality-enum patches to the pinned Soloist build (version-locked, fail-closed on any mismatch), and `check` reports a patch/integrity/architecture manifest. After a fresh Soloist download, SpotOn now runs the patch step automatically and exactly once — patching failures never block playback; Soloist simply runs unpatched with a log warning.
...
- **Build-expiry warning in Settings** — the per-player Soloist status table shows live daemon/pairing state, and a countdown warns before the pinned build's 90-day expiry (with a hard notice once expired).

### Fixed
- **Soloist Browse playback no longer pauses itself right after starting** — the daemon's confirmation of the exact track SpotOn had just requested was misclassified as an "unexpected" track change (a false positive of the autoplay defense), causing spurious corrections and, on the last queued track, an immediate session end with audio stopping after about a second.
- **Browse playback no longer hijacked by a restored Connect session after a daemon restart** — when Soloist restores its previous Spotify session on startup, the stale Connect claim is now released properly, so the next Browse play goes through the Browse path instead of being intercepted by Connect handlers.
- **Cyclic Browse stutter on the Soloist backend turned into a clean (possibly delayed) start** — the stream handoff to the player is now deferred until Soloist demonstrably produces audio (two-stage readiness check, up to 30 s), so LMS's ~10 s premature-end retry loop no longer collides with Soloist's slow decoder startup. The old failure mode (progressive stutter, ~5 s silence, position resetting to 0, repeating endlessly) becomes a bounded buffering wait; stalls longer than 30 s fall back gracefully to the previous behavior.
- **Player no longer hangs in BUFFERING after deselecting the device while paused** — control commands sent to an inactive Connect target now report a distinct status and LMS ejects the stale stream instead of buffering forever. ([#159](https://github.com/stiefenm/spoton/issues/159))
- **Group players no longer restart the song endlessly after pause → skip → play** — a skip while Spotify is paused no longer force-unpauses the group against a silent source. ([#158](https://github.com/stiefenm/spoton/issues/158))
- **Sync-group stutter with mixed player types** — daemons for synced players now start with a 5-second buffer, satisfying LMS's group rebuffer threshold. ([#131](https://github.com/stiefenm/spoton/issues/131))
- **Progress bar no longer lags behind after a mid-song Connect handoff** — the playback position is resynced the moment audio actually starts flowing to LMS. ([#128](https://github.com/stiefenm/spoton/issues/128))
- **Player power state restored after a Connect session ends** — a player that Connect powered on is powered off again when the session ends; players that were already on stay on. ([#151](https://github.com/stiefenm/spoton/issues/151))
- **No more surprise auto-play after an LMS restart** — a dormant Connect session re-announced by a freshly (re)started daemon no longer starts playback on its own; the session stays visible and manually resumable.
- **Connect track skips reconnect in under a second (Soloist)** — the historical ~8s audio gap on Spotify-app skips is gone in the final pipeline (5 measured runs, 0.13–0.88s), and permanent timing instrumentation ships for any recurrence.
- **Search pagination no longer returns empty pages for mixed results** — the spclient search offset guard now compares against the track-filtered result count and delegates to the Web API fallback correctly.

### Removed
- **`sox` dependency and the per-track Soloist launcher** — the per-track `sol` convert-conf rule/content-type and the generated per-track wrapper script are fully retired now that Soloist runs exclusively as the persistent daemon above.
- **`spoton-helper protobuf` subcommand** — superseded by the pure-Perl `ProtobufLite.pm` decoder used directly by the new spclient unification above; no external process is needed to decode collection/recently-played/rootlist data anymore.
